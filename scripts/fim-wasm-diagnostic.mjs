...
function snapshot(sim, outerStep, outerMs, previousHistoryLength) {
  ...
  return {
    ...
    fimSolverMs: fimStepStats?.solver_ms ?? null,
    fimAcceptedSolverMs: fimStepStats?.accepted_solver_ms ?? null,
    fimRetrySolverMs: fimStepStats?.retry_solver_ms ?? null,
    fimAssemblyMs: fimStepStats?.assembly_ms ?? null,
    fimPropertyEvalMs: fimStepStats?.property_eval_ms ?? null,
    fimLinearSolveMs: fimStepStats?.linear_solve_ms ?? null,
    fimLinearPreconditionerMs: fimStepStats?.linear_preconditioner_ms ?? null,
    fimStateUpdateMs: fimStepStats?.state_update_ms ?? null,
...
function printStepSummary(record) {
  console.error(
    [
      ...
      `props_ms=${record.fimPropertyEvalMs == null ? 'n/a' : record.fimPropertyEvalMs.toFixed(1)}`,
      `lin_ms=${record.fimLinearSolveMs == null ? 'n/a' : record.fimLinearSolveMs.toFixed(1)}`,
      `pc_ms=${record.fimLinearPreconditionerMs == null ? 'n/a' : record.fimLinearPreconditionerMs.toFixed(1)}`,
      `upd_ms=${record.fimStateUpdateMs == null ? 'n/a' : record.fimStateUpdateMs.toFixed(1)}`,
      `retry_ms=${record.fimRetrySolverMs == null ? 'n/a' : record.fimRetrySolverMs.toFixed(1)}`,
