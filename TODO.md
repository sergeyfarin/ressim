...
- [ ] Use the new FIM outer-step diagnostics to drive the remaining convergence work instead of relying on trace-only inspection.
  ...
  - Rebuilt wasm evidence on the current head has moved materially beyond the earlier `246` checkpoint baseline: replaying `/tmp/fim-scan-wf12-stats/step-0001.json` now reproduces `substeps=187`, `retries=0/68/0`, `dt=[4.176e-4,4.340e-2]`, and `retry_dom=nonlinear-bad:oil@430` on repeated runs. Treat `187` as the new shipped-path checkpoint baseline for the next CPRW/nonlinear slice, but do not attribute that whole gain to the CPRW metadata cleanup by itself because the current head also includes the broader nonlinear-stabilization work.
  - Current checkpoint diagnosis after that replay: the remaining shelf is still overwhelmingly a producer-corner nonlinear stagnation loop at `cell143` (`oil@430`, with some adjacent `water@429`) on `perf1->well1 inj=false`, not a linear-backend failure. CPR reduction ratios stay tiny when the iterative path is used, but the replay still burns `68` nonlinear retries on repeated near-zero local moves and occasional late `STAGNATION(count=3)` bailouts at the same site.
