...
- [ ] **Combined-scenario simulation E_V is not physically comparable to vertical-scenario E_V** — current 3D `E_V = E_vol / E_A` measures layer occupancy within already-touched XY columns, so near-well early-time values jump to ~0.5/0.6/1.0 by construction. Revisit whether combined geometry should hide simulation E_V or replace it with a streamline/front-based approximation.
	- [x] Phase 1: add combined-geometry path-coordinate helpers in `sweepEfficiency.ts` (injector→producer projected distance, saturation-to-front weight, weighted quantile / front-distance reducer).
	- [x] Phase 2: introduce a geometry-aware simulation metric builder so `vertical` keeps current `E_V = E_vol`, `areal` keeps current `E_V = 1`, and `both` uses a front-distance-based `E_V` instead of `E_vol / E_A`.
	- [ ] Phase 3: find a combined simulation decomposition that keeps the product relation without breaking endpoint behavior. The first path-based `E_A` / `E_vol` attempt regressed badly: `E_A` saturated near 50% and `E_V` reached 100% too early, so combined geometry is currently back to raw swept-cell `E_A` / `E_vol` plus front-based `E_V` only.
	- [x] Phase 4: add regressions for true t=0, one near-injector swept column, uniform-vs-layered combined cases, and “vertical scenario unchanged” behavior.
	- [x] Phase 5: verify runtime and comparison charts stay aligned because both consume the same shared helper path.
