...
Resume note for `R1.3`: live warning surfaces now flow through `warningPolicy` in `ModePanel`, `RunControls`, and the analytical caveat banner. Remaining warning islands to clean or retire are legacy shell-era components: `src/lib/ui/InputsTab.svelte`, `src/lib/ui/PresetCustomizeShell.svelte`, and `src/lib/ui/DynamicControlsPanel.svelte`.
