...
- Current linear-track interpretation:
  ...
  - provisional larger-case evidence on the same modified tree is directionally encouraging and should drive the next validation round: `20x20x3` water currently lands at `substeps=13`, `retries=0/6/0`, while `20x20x3` gas already lands at `substeps=4`, `retries=0/2/0`. These are not baseline claims yet because they were not replayed against a committed pre-change revision, but they support continuing this producer-hotspot line rather than reverting it immediately
- Current interpretation:
