...
- Over-threshold CPR cases are still open on current head:
  ...
  - new per-restart failure traces show the remaining pattern directly: the first 2–3 restarts often improve by orders of magnitude (`upd=y`), then the solve falls into a short-restart tail at tiny `outer_res` / `prec_res` where the restart candidate stops helping (`upd=n`) and can be worse than the current iterate even when the Givens residual estimate is still very small
...
- Current linear-track interpretation:
  ...
  - that means the next over-threshold slice should stay inside Phase 2 but narrow further: target the remaining asymptotic short-restart tail after the first successful restart improvements, before mixing the result back into the shared nonlinear-controller track
- Current interpretation:
