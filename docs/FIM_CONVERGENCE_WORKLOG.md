...
- Current linear-track interpretation:
  ...
  - updated implication: the next fix should stay inside the GMRES tail family, but it must be narrower than “exact-dense vs over-threshold”. The likely target is a more specific exact-dense control condition keyed to the short `22x22x1` tail geometry or restart shape, while preserving the current tail behavior on both `23x23x1` and `12x12x3`
- Current interpretation:
