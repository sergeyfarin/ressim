...
- **Scenario model**: `src/lib/catalog/scenarios.ts` is the single source of truth for all predefined scenarios. 18 entries currently; S1 redesign (REFACTOR.md Phase 2) will consolidate to ~6 canonical scenarios with multi-dimension sensitivity.
- **Input UI**: `src/lib/ui/modes/ScenarioPicker.svelte` is the primary input surface. Domain tabs (Waterflood / Sweep / Depletion / Gas) and sensitivity dimension selector coming in S1.
