# FIM Convergence Worklog Archive

This file is an archived snapshot of the former live worklog before the 2026-04-08 cleanup pass.

- Use `docs/FIM_STATUS.md` for the current consolidated solver status.
- Use `docs/FIM_CONVERGENCE_WORKLOG.md` for active investigations only.
- Use this archive for the full March 2026 through 2026-04-06 narrative that was trimmed out of the live worklog.

## Validation Update - 2026-04-08 coarse-pressure-solver note narrowed to representative exact-dense cases

- Motivation from the current doc audit:
  - the older worklog conclusion around CPR coarse-solver quality was being quoted too broadly as if the item were generally closed
  - current head still needs the distinction between:
    - representative hard 3D shelves whose extracted coarse systems stay below the exact-dense threshold
    - larger over-threshold shelves where the CPR coarse stage falls back to the ILU defect-correction path
- Code recheck on current head:
  - `src/lib/ressim/src/fim/linear/gmres_block_jacobi.rs` still uses two coarse-pressure paths:
    - exact dense inverse when `pressure_rows <= 512`
    - ILU defect-correction otherwise
  - no solver code change was needed for this audit; the problem here was stale wording, not a newly found implementation bug
- Focused validation:
  - representative hard 3D shelf:
    - `node scripts/fim-wasm-diagnostic.mjs --preset water-pressure --grid 12x12x3 --steps 1 --dt 1 --diagnostic step --no-json | rg "cpr=\[|FIM retry summary|FIM step done|step=|retry=\[class="`
    - current trace still shows the extracted coarse system on the exact path:
      - `cpr=[rows=434 solver=dense apps=12 avg_rr=1.148e-13 last_rr=3.754e-14]`
      - final shelf headline remains reservoir-row dominated: `FIM step done: 129 substeps`, `FIM retry summary: linear-bad=0 nonlinear-bad=35 mixed=0`
  - bounded over-threshold check:
    ...
    - first iterative CPR pass on the over-threshold case now shows the actual large-case behavior instead of the representative exact-dense one:
      - `cpr=[rows=531 solver=ilu apps=11 avg_rr=7.557e-2 last_rr=7.896e-2]`
      - later iterations also show repeated linear failure and dense-LU fallback on the same `1591`-row system
- Interpretation:
  - the old representative-case statement is still valid in its narrow scope: on cases like `wf_p_12x12x3`, the coarse stage is solved essentially to machine precision and the remaining shelf is not explained by an under-solved coarse system
  - but treating that as a general closure for coarse-pressure-solver quality was documentation lag
  - once the extracted coarse system moves above the exact-dense threshold, coarse-solver quality is active again on current head; the ILU defect-correction path is not reaching machine-precision reduction and can still hand the Newton loop a materially weaker pressure correction
  - updated conclusion: keep the earlier representative-case note, but narrow it explicitly to exact-dense coarse systems and keep coarse-pressure-solver quality open for over-threshold CPR cases

## Validation Update - 2026-04-08 repeated reservoir-hotspot cooldown now caps any regrowth path

- Motivation after the well-Schur recheck:
  - the current hard water and gas shelves are both reservoir-row dominated `nonlinear-bad` retries, not the older well/perforation-manifold failure mode
  - the existing hotspot cooldown only capped accepted-step growth when the limiter label was `max-growth`
  - that missed the shipped gas path, where repeated hotspot retries were regrowing under `growth=newton-iters` rather than `growth=max-growth`
- Change made in `src/lib/ressim/src/fim/timestep.rs`:
  - repeated hotspot memory now caps any accepted-step regrowth factor above `1.0`, regardless of which limiter produced it
  - the retry-cooldown clean-success budget now scales with hotspot repeat count instead of adding only one extra hold step at the highest repeat levels
  - focused regression coverage added for:
    - repeated hotspot capping `newton-iters` regrowth as well as `max-growth`
    - repeated hotspot increasing the clean-success hold budget
- Focused validation:
  - `cargo test --manifest-path src/lib/ressim/Cargo.toml repeated_hotspot_ -- --nocapture`
  - `cargo test --manifest-path src/lib/ressim/Cargo.toml alternating_gas_cells_with_shared_injector_symmetry_site_count_as_same_hotspot -- --nocapture`
  - `cargo test --manifest-path src/lib/ressim/Cargo.toml spe1_fim_first_steps_converge_without_stall -- --nocapture`
  - all passed

### Measured result on canonical hard water shelf
...
- Before:
  ...
  - `retries=0/44/0`
  - `retry_ms=32917.0`
  - `retry_dom=nonlinear-bad:oil@430`
- After:
  - `outer_ms=118915.1`
  - `substeps=129`
  - `retries=0/35/0`
...
- this is a real win on the shared reservoir-row water shelf: fewer retries, fewer accepted substeps, and materially less time spent in retry work
- the dominant hotspot did not move, which is consistent with the change being controller-side rather than physics-side

### Measured result on shipped gas shelf

- Diagnostic:
  - `node scripts/fim-wasm-diagnostic.mjs --preset gas-rate --grid 10x10x3 --steps 6 --dt 0.25 --diagnostic outer --no-json`
- Result after the change:
  - step 1: `retries=0/3/0`, `growth=max-growth`
  - step 2: `retries=0/2/0`, `growth=hotspot-repeat`
  - step 3: `retries=0/2/0`, `growth=newton-iters`
  - step 4: `retries=0/2/0`, `growth=newton-iters`
  - step 5: `retries=0/2/0`, `growth=hotspot-repeat`
  - step 6: `retries=0/2/0`, `growth=newton-iters`
  - wall clock stayed roughly flat (`1:14.59` vs the previously tracked `1:13.66` class)

Interpretation:

- the policy now clearly reaches the gas path that was previously bypassing hotspot memory through the `newton-iters` limiter
- however, this is only a partial leverage point on gas: retry counts did not materially improve on the shipped replay, so the next gas-side slice likely needs a broader notion of repeated reservoir hotspot memory than the current exact-site grouping alone
- current conclusion: keep this change because it materially improves the shared hard water shelf and correctly generalizes hotspot cooldown to non-`max-growth` regrowth paths, but do not treat it as the full reservoir-row fix for gas

## Validation Update - 2026-04-08 hotspot-repeat/newton-iters metric surfaced and gas cooldown memory broadened

- Follow-up goals from the previous shared reservoir-hotspot slice:
  - make the controller effect directly measurable instead of inferring it from `growth=` labels in traces
  - broaden gas cooldown memory beyond exact hotspot site, because the shipped gas shelf was still moving between injector-side gas rows even after exact-site symmetry cleanup
- Changes made:
  - `src/lib/ressim/src/reporting.rs`
    - `FimStepStats` now carries `hotspot_repeat_suppressed_newton_iters_growth_count`
  - `scripts/fim-wasm-diagnostic.mjs`
    - summary/outer/step output now prints that metric as `hotspot_newton_caps=`
  - `src/lib/ressim/src/fim/timestep.rs`
    - cooldown memory now distinguishes exact retry diagnostics from a broader memory key
    - for gas-dominated shelves, cooldown comparison now uses a broader reservoir-region key:
      - `GasInjectorSymmetry` sites are grouped by `injector_well_index + vertical_offset`
      - non-symmetry gas fallbacks use gas-family-level memory instead of exact-cell memory
    - exact hotspot site is still retained in diagnostics and trace output; only the cooldown comparison key was broadened
  - focused regression coverage added for:
    - same-injector gas hotspots with different in-plane offsets sharing one memory region
    - different vertical offsets not sharing the same gas memory region
    - non-symmetry gas failures falling back to gas-family memory

Validation:

- Focused Rust tests passed:
  - `cargo test --manifest-path src/lib/ressim/Cargo.toml gas_hotspots_in_same_injector_region_count_as_same_memory_region -- --nocapture`
  - `cargo test --manifest-path src/lib/ressim/Cargo.toml gas_hotspots_on_different_vertical_offsets_do_not_share_memory_region -- --nocapture`
  - `cargo test --manifest-path src/lib/ressim/Cargo.toml gas_family_fallback_memory_groups_non_symmetry_gas_failures -- --nocapture`
  - `cargo test --manifest-path src/lib/ressim/Cargo.toml repeated_hotspot_also_caps_newton_limited_regrowth -- --nocapture`
  - `cargo test --manifest-path src/lib/ressim/Cargo.toml spe1_fim_first_steps_converge_without_stall -- --nocapture`

### Measured result on canonical hard water shelf with the new metric
...
- the new field is useful immediately: the hard water shelf now reports exactly how often hotspot-repeat overrode a raw `newton-iters` regrowth path (`12` times on the current day-1 shelf)
- that confirms the controller is no longer acting only through obvious `growth=hotspot-repeat` headlines; it is also reshaping the internal accepted-step growth decisions during the shelf

### Measured result on shipped gas shelf after broader gas memory

- Diagnostic:
  - `node scripts/fim-wasm-diagnostic.mjs --preset gas-rate --grid 10x10x3 --steps 6 --dt 0.25 --diagnostic outer --no-json`
- Result after the broader gas memory change:
  - step 1: `retries=0/3/0`, `growth=max-growth`, `hotspot_newton_caps=0`
  - step 2: `retries=0/2/0`, `growth=hotspot-repeat`, `hotspot_newton_caps=4`
  - step 3: `retries=0/2/0`, `growth=hotspot-repeat`, `hotspot_newton_caps=4`
  - step 4: `retries=0/2/0`, `growth=hotspot-repeat`, `hotspot_newton_caps=4`
  - step 5: `retries=0/2/0`, `growth=hotspot-repeat`, `hotspot_newton_caps=4`
  - step 6: `retries=0/2/0`, `growth=hotspot-repeat`, `hotspot_newton_caps=4`
  - wall clock improved to `1:11.18` from the previous `1:14.59` class

Interpretation:

- this broader gas memory change did what the previous exact-site-only policy could not: the gas shelf is now held on `hotspot-repeat` consistently from step 2 onward instead of slipping back to `newton-iters`
- retry counts are still unchanged on the shipped short replay, so this is not yet the full gas fix
- but the new metric proves the controller is engaging on exactly the path we wanted to affect, and the modest wall-clock improvement suggests the broader gas memory is directionally correct
- next follow-up should target what still remains after growth suppression is working: either stronger cutback retention across successive gas-region failures or a larger-grain memory that survives across adjacent outer steps without resetting after a single clean accept
