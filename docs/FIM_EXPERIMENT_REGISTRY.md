...
| FIM-LINEAR-005 | Linear/CPR | CPR pressure restriction should not be unconditional water row | `fim/linear/gmres_block_jacobi.rs` | Medium water, OPM comparison | Added verbose-only measurement probe comparing row-0 Schur against summed, diagonal-balanced, dominant-row, and normalized local-Schur restrictions; no live solver behavior changed | DIAGNOSTIC | Run failing heavy/medium traces and promote only if post-preconditioner residual consistently identifies a better pressure row | `docs/FIM_LINEAR_SOLVER_AUDIT.md` "Finding 3"; `docs/FIM_CPR_IMPROVEMENT_PLAN.md` |
