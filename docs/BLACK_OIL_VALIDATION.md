...
| Gas reporting / `dep_gas_pz` | [#25](https://github.com/sergeyfarin/ressim/issues/25) | **no** | closure `+0.0109` (issue claimed `+0.0885`); recovery `0.9280` vs documented `0.928` | `record_fim_step_report` gas conversion | scenario's own volumetric GIIP | **not reproduced — close.** Superseded 2026-09-25 (#57): #25 was real. The harness billed each step at its last substep's rate, which hid it; integrating every substep, the closure was `+0.0885` exactly as claimed, with recovery above 1. The cause was the engine taking this dry gas as ideal above the table's 20 bar bubble point. With the table's own gas rows the closure is `+0.00015` and cumulative gas matches Flow's |
