...
export const SCENARIOS: Scenario[] = [
    ...
    // ─── Sweep Efficiency Scenarios ────────────────────────────────────────────

    {
        key: 'sweep_areal_mobility',
        label: 'Areal Sweep – Mobility',
        description: 'Five-spot areal sweep efficiency at breakthrough depends strongly on the mobility ratio M. E_A(BT) ≈ 0.70 at M = 1, drops sharply for unfavourable M > 1. — Craig (1971)',
        scenarioClass: 'waterflood',
        chartPreset: 'waterflood',
        params: {
            ...BL_A_BASE_PARAMS,
        ...
        sensitivity: {
            key: 'mobility_sweep',
            label: 'Mobility ratio (M)',
            description: 'Vary μ_o to explore how the end-point mobility ratio controls areal sweep efficiency and pattern breakthrough. Both simulation and analytical update.',
            variants: [
                {
                    key: 'M_favorable',
                    label: 'M ≈ 0.5 (μ_o = 0.25 cp)',
                    description: 'Favourable mobility — high sweep, late breakthrough, piston-like displacement.',
                ...
                },
                {
                    key: 'M_unfavorable',
                    label: 'M ≈ 10 (μ_o = 5 cp)',
                    description: 'Strongly unfavourable — viscous fingering dominates, early BT, poor sweep.',
                    paramPatch: { mu_o: 5.0 },
                    affectsAnalytical: true,
                },
            ],
    ...
    {
        key: 'sweep_areal_residual',
        label: 'Areal Sweep – Residual',
        description: 'Residual oil saturation affects the fractional flow curve shape, shifting the Welge tangent and hence the effective displacement efficiency. Sweep efficiency responds via the mobility ratio endpoint shift. — Willhite (1986)',
        scenarioClass: 'waterflood',
        chartPreset: 'waterflood',
        params: {
            ...BL_A_BASE_PARAMS,
        ...
        sensitivity: {
            key: 'residual_sweep',
            label: 'Residual oil saturation (S_or)',
            description: 'S_or affects both displacement efficiency ceiling and the endpoint mobility ratio, influencing sweep. Both simulation and analytical update.',
            variants: [
                {
                    key: 'sor_low_sweep',
                    label: 'S_or = 0.05',
    ...
    {
        key: 'sweep_vertical_vdp',
        label: 'Vertical Sweep – V_DP',
        description: 'Vertical sweep efficiency for non-communicating layers with piston-like displacement. Higher Dykstra-Parsons coefficient V_DP → more heterogeneity → earlier breakthrough in fast layers, lower E_V. — Dykstra & Parsons (1950)',
        scenarioClass: 'waterflood',
        chartPreset: 'waterflood',
        params: {
            ...BL_A_BASE_PARAMS,
            ...
            uniformPermY: 100,
            uniformPermZ: 10,
            // Moderate heterogeneity: VDP ≈ 0.5
        ...
        sensitivity: {
            key: 'vdp_variation',
            label: 'Permeability heterogeneity (V_DP)',
            description: 'Compare uniform, moderate, and extreme layer heterogeneity — only the simulation changes; the analytical sweep solution uses the actual layer data.',
            variants: [
                {
                    key: 'vdp_uniform',
                    label: 'Uniform (V_DP ≈ 0)',
                    description: 'All layers have equal permeability — all fronts advance equally, E_V = 1 at BT.',
                    paramPatch: {
                        permMode: 'uniform',
                        uniformPermX: 100,
                        uniformPermY: 100,
                ...
                {
                    key: 'vdp_moderate',
                    label: 'Moderate (V_DP ≈ 0.5)',
                    description: '5:1 perm range across layers — typical clastic reservoir heterogeneity.',
                    paramPatch: {},
                    affectsAnalytical: false,
                },
                {
                    key: 'vdp_extreme',
                    label: 'Extreme (V_DP ≈ 0.8)',
                    description: '50:1 perm range — thief zone dominates, very early BT in fastest layer.',
                    paramPatch: {
...

    {
        ...
        description: 'Overall volumetric sweep E_vol = E_A × E_V. Multiply areal (pattern geometry + mobility) by vertical (layered heterogeneity) to see the total fraction of reservoir contacted. — Craig (1971), Dykstra & Parsons (1950)',
        scenarioClass: 'waterflood',
        chartPreset: 'waterflood',
        params: {
            ...BL_A_BASE_PARAMS,
        ...
        sensitivity: {
            key: 'combined_sweep',
            label: 'Mobility × Heterogeneity',
            description: 'Combine unfavourable mobility (low E_A) with layer heterogeneity (low E_V) to see the full volumetric sweep penalty. Both simulation and analytical update.',
            variants: [
                {
                    key: 'combined_ideal',
                    label: 'Ideal (M ≈ 1, uniform layers)',
                    description: 'M = 1 + uniform permeability — best possible sweep.',
                    paramPatch: {
                        mu_o: 0.5,
                        permMode: 'uniform',
                        uniformPermX: 100,
                ...
                },
                {
                    ...
                    label: 'Moderate (M ≈ 2, V_DP ≈ 0.5)',
                    description: 'Typical field conditions — moderate mobility and heterogeneity.',
                    paramPatch: {},
                    affectsAnalytical: true,
                },
                {
                    key: 'combined_poor',
                    label: 'Poor (M ≈ 10, V_DP ≈ 0.8)',
                    description: 'Viscous oil in a heterogeneous reservoir — worst-case volumetric sweep.',
                    paramPatch: {
                        mu_o: 5.0,
                        layerPermsX: [500, 150, 50, 20, 10],
                        layerPermsY: [500, 150, 50, 20, 10],
