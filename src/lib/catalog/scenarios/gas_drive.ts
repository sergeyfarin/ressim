...
export const gas_drive: Scenario = {
    ...
    analyticalMethodSummary: 'Tarner–Tracy volumetric tank model — couples black-oil material balance to the instantaneous producing-GOR relation at each average-pressure step. It predicts recovery, cumulative oil, and GOR but deliberately omits spatial pressure gradients, gravity segregation, and well deliverability; the bundled OPM Flow curves remain an optional benchmark overlay.',
    analyticalMethodReference: 'Tarner (1944), solution-gas-drive performance prediction; Tracy (1955), simplified material-balance equation forms; Standing (1979), Notes on Calculating Solution Gas Drive Material Balance. Quantitative simulator grading: docs/THREE_PHASE_VALIDATION.md.',
    referenceSources: [{ kind: 'opm-flow', artifactKeys: ['gas_drive'], defaultVisible: false }],
    chartLayoutKey: 'gas',
    chartLayoutPatch: {
        rateChart: {
            panels: {
                recovery: { curveKeys: ['recovery-factor-primary', 'recovery-factor-reference'] },
                gor: { curveKeys: ['gor-sim', 'gor-reference'] },
                cumulative: { curveKeys: ['cum-oil-sim', 'cum-oil-reference'] },
            },
        },
    },
    defaultSensitivityDimensionKey: 'sg_init',
    capabilities: {
        analyticalMethod: 'tarner-tracy',
        hasInjector: false,
