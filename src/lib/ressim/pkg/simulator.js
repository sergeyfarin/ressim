...
export class ReservoirSimulator {
     ...
     * @param {any} config_js
     */
    setSweepConfig(config_js) {
        const ret = wasm.reservoirsimulator_setSweepConfig(this.__wbg_ptr, config_js);
        if (ret[1]) {
            throw takeFromExternrefTable0(ret[0]);
        }
    }
...
const ReservoirSimulatorFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
    : new FinalizationRegistry(ptr => wasm.__wbg_reservoirsimulator_free(ptr, 1));
