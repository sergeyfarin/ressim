...
class SimulationStoreImpl {
    ...
    applyCaseParams(params: Record<string, any>) {
        ...
        this.nx = Math.max(1, Math.round(Number(resolved.nx) || 1));
        this.ny = Math.max(1, Math.round(Number(resolved.ny) || 1));
        this.nz = Math.max(1, Math.round(Number(resolved.nz) || 1));
        this.cellDx = Number(resolved.cellDx) || 10;
        this.cellDy = Number(resolved.cellDy) || 10;
        this.cellDz = Number(resolved.cellDz) || 1;
        this.delta_t_days = Number(resolved.delta_t_days) || 0.25;
        this.steps = Math.max(1, Math.round(Number(resolved.steps) || 20));
        this.max_sat_change_per_step = Number(resolved.max_sat_change_per_step) || 0.1;
        this.initialPressure = Number(resolved.initialPressure) || 300;
        this.initialSaturation = Number(resolved.initialSaturation) || 0.3;
        this.mu_w = Number(resolved.mu_w) || 0.5;
        this.mu_o = Number(resolved.mu_o) || 1.0;
        this.c_o = Number(resolved.c_o) || 1e-5;
        this.c_w = Number(resolved.c_w) || 3e-6;
        this.rock_compressibility = Number(resolved.rock_compressibility) || 1e-6;
        this.depth_reference = Number(resolved.depth_reference) || 0;
        this.volume_expansion_o = Number(resolved.volume_expansion_o) || 1.0;
        this.volume_expansion_w = Number(resolved.volume_expansion_w) || 1.0;
        this.rho_w = Number(resolved.rho_w) || 1000;
        this.rho_o = Number(resolved.rho_o) || 800;
        this.s_wc = Number(resolved.s_wc) || 0.1;
        this.s_or = Number(resolved.s_or) || 0.1;
        this.n_w = Number(resolved.n_w) || 2.0;
        this.n_o = Number(resolved.n_o) || 2.0;
        this.k_rw_max = Number(resolved.k_rw_max) || 1.0;
        this.k_ro_max = Number(resolved.k_ro_max) || 1.0;
        this.well_radius = Number(resolved.well_radius) || 0.1;
        this.well_skin = Number(resolved.well_skin) || 0;
        this.max_pressure_change_per_step = Number(resolved.max_pressure_change_per_step) || 75;
        this.max_well_rate_change_fraction = Number(resolved.max_well_rate_change_fraction) || 0.75;
        this.gravityEnabled = Boolean(resolved.gravityEnabled);
        this.capillaryEnabled = resolved.capillaryEnabled !== false;
        this.capillaryPEntry = Number(resolved.capillaryPEntry) || 0;
        this.capillaryLambda = Number(resolved.capillaryLambda) || 2;
        ...
        this.injectorBhp = Number(resolved.injectorBhp) || 400;
        this.producerBhp = Number(resolved.producerBhp) || 100;
        this.targetInjectorRate = Number(resolved.targetInjectorRate) || 350;
        this.targetProducerRate = Number(resolved.targetProducerRate) || 350;
        ...
        this.injectorI = Number(resolved.injectorI) || 0;
        this.injectorJ = Number(resolved.injectorJ) || 0;
        this.producerI = Number(resolved.producerI) || this.nx - 1;
        this.producerJ = Number(resolved.producerJ) || 0;
