...
- A method with no reference solution (`'none'`, `'digitized-reference'`) has no curve slots and emits nothing. It must never fall through to another method's overlay path — that bug shipped for a while and is now pinned by a regression test in `referenceComparisonModel.test.ts`.
