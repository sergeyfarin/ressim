...
4. `docs/FIM_CONVERGENCE_WORKLOG.md` — active hypotheses and traces (Phase 9 onward — component-isolation lab, Phase 10's OPM `cprw` bundle, Phase 11's well-Schur-elimination/OSC-DETECT work).
5. `docs/FIM_CONVERGENCE_ARCHIVE_2026-04-08_to_2026-07-03.md` — water/gas shelf investigations, Phase 5 AD-assembler cutover, Phase 6, Phase 7 (all 5 sub-phases), Phase 8, Hypothesis C. `docs/FIM_CONVERGENCE_ARCHIVE_2026-03_to_2026-04-06.md`, `docs/FIM_HISTORY_2026-03.md` — older still.
...
- `FIM-NEWTON-007` (refuted): the `water@387`/`cell129` failure traces to `appleyard_damping_breakdown`'s fw-inflection trust-region chop degenerating to exactly `0.0` when a cell's Sw sits essentially at the inflection point (confirmed precisely via trace: `bind=sw_inflection@cell115`, `final=0.0000`). Three variants fixing this exact degenerate case (additive margin, `max()` floor, skip-below-threshold) were each built, unit-tested (each correctly fixed the isolated case), and live-tested — **all three regressed the heavy case similarly** (`62→263`, `62→263`, `62→238` substeps), and each brought back the just-fixed `perf@1299` oscillation rather than fixing anything. Reverted cleanly (confirmed bit-identical to the pre-experiment `62`-substep state). **Do not re-attempt a local chop-formula change at this exact site** — the heavy case's Newton trajectory is evidently sensitive enough there that local fixes reliably perturb the path into a different already-solved problem; any future attempt needs to first explain *why* this site is this sensitive, not just patch the formula again.
