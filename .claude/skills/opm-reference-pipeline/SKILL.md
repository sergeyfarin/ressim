...
- `tools/opm_flow/opm_flow_tool/cases.py` — `OpmCase` definitions. Seventeen cases as of 2026-09-25: `wf_bl1d`, `spe1_gas_injection`, `gas_drive`, `wf_gravity`, `wf_numerics` / `wf_numerics_fine`, `dep_gas_pz` / `dep_gas_pz_geopressured`, `gas_injection`, `dep_pvt_correlation` / `dep_pvt_lab_report`, and the six two-phase waterfloods `sweep_areal`, `sweep_vertical`, `sweep_crossflow`, `sweep_combined`, `wf_capillary`, `wf_gravity_stability` (`_catalog_waterflood_case`). A pair drawn on one chart needs distinct `curveKey`s (`opm-fine-`, `opm-geo-`, `opm-lab-` prefixes) and says its rung in every label.
